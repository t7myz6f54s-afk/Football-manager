...
const NAV=[
  ...
  ["sep"],["table","","Table"],["comps","","Competitions"],["history","","History"],["calendar","","Calendar"],["inbox","","News"],
...
function godPlan(pl){
  if(!pl) return "";
  let h=`<div style="padding:10px;border-radius:12px;background:linear-gradient(135deg,#141208,#1a1810);border:1px solid rgba(255,204,51,.28)"><div style="font-size:9px;letter-spacing:.14em;font-weight:950;color:var(--gold)">THE VERDICT</div><div style="font-size:12.5px;font-weight:800;margin-top:5px;line-height:1.45">${esc(pl.verdict||"")}</div></div>`;
  if((pl.xi||[]).length){
    h+=`<div class="card tight" style="margin-top:8px"><div class="sec-h"><h3>Best XI</h3><button class="btn sm primary" onclick="godXI()">Select ▶</button></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-top:8px">${pl.xi.map(x=>`<div style="display:flex;gap:6px;align-items:center;padding:4px 8px;border-radius:8px;background:var(--panel2)"><span class="pos" style="min-width:30px">${esc(x.pos)}</span><span style="flex:1;min-width:0;font-size:10.5px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${esc(x.why)}">${esc(x.name)}</span><span style="font-size:9px;color:var(--tx3);flex:none">CA ${x.ca}</span></div>`).join("")}</div><div class="small muted" style="font-size:9px;margin-top:6px">Tap a player for the reasoning · one line, eleven choices</div></div>`;
  }
  if(pl.tactics){
    const t=pl.tactics;
    h+=`<div class="card tight" style="margin-top:8px"><div class="sec-h"><h3>Plan v ${esc(t.opp||"opponent")}</h3><button class="btn sm primary" onclick="godTactics()">Apply ▶</button></div><div style="font-size:11.5px;font-weight:900;margin-top:6px">${esc(t.mentality)} · ${esc(t.formation)}</div><div style="display:grid;gap:3px;margin-top:6px">${(t.why||[]).map(w=>`<div style="font-size:10.5px;color:var(--tx2);padding-left:10px;position:relative;line-height:1.35"><span style="position:absolute;left:0;color:var(--gold)">·</span>${esc(w)}</div>`).join("")}</div></div>`;
  }
  if((pl.threats||[]).length){
    h+=`<div class="card tight" style="margin-top:8px"><h3>Threats to manage</h3><div style="display:grid;gap:3px;margin-top:6px">${pl.threats.map(t=>`<div style="display:flex;gap:8px;align-items:center;padding:5px 8px;border-radius:8px;background:var(--panel2)"><span class="pos" style="min-width:30px">${esc(t.pos)}</span><span style="flex:1;font-size:10.5px;font-weight:800">${esc(t.name)}</span><span style="font-size:9px;color:var(--tx3)">${esc(t.why)}</span></div>`).join("")}</div></div>`;
  }
  if((pl.sign||[]).length||(pl.rejects||[]).length){
    h+=`<div class="card tight" style="margin-top:8px"><h3>Transfer intelligence</h3><div style="display:grid;gap:6px;margin-top:8px">${(pl.sign||[]).map(s=>`<div style="display:flex;gap:8px;align-items:center;padding:7px 8px;border-radius:10px;background:var(--panel2);border:1px solid var(--line)"><div style="flex:1;min-width:0"><b style="font-size:11px">${esc(s.name)}</b> <span class="small muted">${esc(s.pos)} · ${s.age}y · CA ${s.ca}</span><div class="small muted" style="font-size:9.5px;margin-top:1px">${esc(s.club)} · ~${money(s.value)} — ${esc(s.why)}</div></div><button class="btn sm primary" onclick="godBid(${s.pid},${Math.round(s.value)},${Math.round(s.wage)})">Bid</button></div>`).join("")}${(pl.rejects||[]).map(r=>`<div style="padding:6px 8px;border-radius:8px;background:rgba(255,59,74,.06);border:1px solid rgba(255,59,74,.14)"><b style="font-size:10.5px;color:#ff9aa2">✗ ${esc(r.name)} <span class="small muted">(${esc(r.club)}, ${esc(r.pos)})</span></b><div class="small muted" style="font-size:9.5px;margin-top:1px">${esc(r.why)}</div></div>`).join("")}</div></div>`;
  }
  if(pl.squad){
    const sq=pl.squad, bits=[];
    if((sq.depth||[]).length) bits.push(...sq.depth.map(d=>`<span class="tag URGENT" style="font-size:9px">${esc(d)}</span>`));
    if((sq.contracts||[]).length) bits.push(`<span class="tag" style="font-size:9px;background:rgba(255,176,46,.14);color:var(--amber)">Renew: ${sq.contracts.map(esc).join(", ")}</span>`);
    (sq.sell||[]).forEach(s=>bits.push(`<span class="tag" style="font-size:9px;background:rgba(255,59,74,.1);color:#ff9aa2">Sell: ${esc(s.name)} — ${esc(s.why)}</span>`));
    (sq.develop||[]).forEach(d=>bits.push(`<span class="tag" style="font-size:9px;background:rgba(77,158,255,.12);color:#9cc3ff">Develop: ${esc(d.name)} (${d.age}y) — ${esc(d.why)}</span>`));
    if(bits.length) h+=`<div class="card tight" style="margin-top:8px"><h3>Squad</h3><div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:6px">${bits.join("")}</div></div>`;
  }
  if(pl.fixtures&&pl.fixtures.next&&pl.fixtures.next.length){
    h+=`<div class="card tight" style="margin-top:8px"><h3>Fixture load</h3><div style="margin-top:6px;display:grid;gap:3px">${pl.fixtures.next.slice(0,3).map(f=>`<div style="display:flex;gap:8px;font-size:10.5px"><span style="color:var(--tx3);flex:none;width:64px">${f.date.slice(5)}</span><span style="font-weight:800;flex:1">${esc(f.text)}</span><span class="small muted">${esc(f.comp||"")}</span></div>`).join("")}</div>${pl.fixtures.advice?`<div class="small muted" style="font-size:9.5px;margin-top:6px;color:var(--tx2)">${esc(pl.fixtures.advice)}</div>`:""}</div>`;
  }
  if(pl.finance){
    h+=`<div class="card tight" style="margin-top:8px"><h3>Finance</h3><div class="kv" style="margin-top:4px"><span>Budget</span><b>${money(pl.finance.budget)}</b></div><div class="kv"><span>Wage headroom</span><b>${money(pl.finance.head)}/yr</b></div><div class="small muted" style="font-size:9.5px;margin-top:5px;line-height:1.4">${(pl.finance.why||[]).map(esc).join(" · ")}</div></div>`;
  }
  return h;
}
function godCard(adv){
  if(!adv||!adv.ok) return "";
  const items=adv.on?(adv.items||[]).map(it=>`<div class="god-it"><span class="tag ${it.tag==="INBOX"||it.tag==="BOARD"?"URGENT":""}">${esc(it.tag)}</span><div style="flex:1"><b style="font-size:11px">${esc(it.t)}</b><div class="small muted" style="font-size:10px">${esc(it.b)}</div></div>${it.go?`<button class="btn sm" onclick="go('${it.go}')">OPEN</button>`:""}</div>`).join(""):"";
  return `<div class="card god ${adv.on?"on":""}" style="margin-top:10px"><div style="display:flex;align-items:center;gap:10px"><div class="god-m">GF</div><div style="flex:1"><div style="font-size:9px;letter-spacing:.12em;font-weight:900;color:var(--tx3)">GODFATHER MODE</div><div class="small muted" style="font-size:10px">${adv.on?"Reads your club — verdict, XI, plan, market":"Off"}</div></div><label class="sw"><input type="checkbox" ${adv.on?"checked":""} onchange="toggleGod(this.checked)"><span></span></label></div>${adv.on?`<div style="margin-top:10px;display:grid;gap:8px">${godPlan(adv.plan)}${items?`<div style="display:grid;gap:6px">${items}</div>`:""}</div>`:`<div style="margin-top:8px"><p class="small muted">Off.</p></div>`}</div>`;
}
async function godXI(){ const adv=await api.get("/api/advice"); const ids=(adv.plan||{}).xi_ids||[]; if(!ids.length) return toast("No XI",4000); const r=await api.post("/api/match/select",{ids}); toast(r.ok?"Godfather XI selected":"Failed",4000); if(r.ok) go("match"); }
...
function careerCard(p){
  const rows=(p.career||[]).map(c=>`<div style="display:grid;grid-template-columns:56px 1fr 26px 26px 26px 44px;gap:6px;padding:5px 8px;border-radius:8px;background:var(--panel2);font-size:10px;align-items:center"><span style="font-family:var(--ff-mono);font-weight:800">${c.season}/${String(c.season+1).slice(2)}</span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:800">${esc(c.club)}</span><span style="text-align:center;color:var(--tx3)">${c.apps}</span><span style="text-align:center;color:var(--acc);font-weight:900">${c.goals}</span><span style="text-align:center;color:var(--tx3)">${c.assists}</span><span style="text-align:center;font-family:var(--ff-mono)">${c.avg_rating?c.avg_rating.toFixed(2):"—"}</span></div>`).join("");
  const aw=(p.career_awards||[]).map(a=>`<span style="font-size:9px;padding:2px 8px;border-radius:999px;background:rgba(255,204,51,.1);color:var(--gold);font-weight:900">${a.season} · ${esc(a.name)}</span>`).join("");
  if(!(p.career&&p.career.length)&&!(p.career_awards&&p.career_awards.length)) return "";
  return `<div class="card tight" style="margin-top:8px"><div style="display:flex;justify-content:space-between;align-items:center"><h3>Career record</h3><span class="small muted" style="font-size:9px">apps · goals · assists · rating</span></div><div style="display:grid;gap:3px;margin-top:8px">${rows||'<p class="small muted" style="padding:4px">No recorded seasons yet</p>'}</div>${aw?`<div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:8px">${aw}</div>`:""}</div>`;
}
...
function godMatchBox(g){
  return `<div class="card god on" style="margin-top:8px"><div style="display:flex;gap:10px;align-items:flex-start"><div class="god-m" style="flex:none">GF</div><div style="flex:1;min-width:0"><b style="font-size:11.5px">${esc(g.verdict||"")}</b>${(g.threats||[]).length?`<div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:6px">${g.threats.map(t=>`<span style="font-size:9.5px;padding:2px 8px;border-radius:999px;background:rgba(255,59,74,.1);color:#ff9aa2;font-weight:800">${esc(t.name)} ${esc(t.pos)} · ${esc(t.why)}</span>`).join("")}</div>`:""}${g.xi_note?`<div style="margin-top:6px;font-size:10.5px;color:var(--amber);font-weight:800">⚠ ${esc(g.xi_note)}</div>`:""}</div></div></div>`;
}
...
function paintGod(st){
  const el=$("#lv-god"); if(!el) return;
  const g=st&&st.godfather; if(!g){ el.innerHTML=""; return; }
  el.innerHTML=`<div style="display:flex;gap:8px;align-items:center;padding:7px 12px;margin:0 0 4px;background:rgba(255,204,51,.06);border:1px solid rgba(255,204,51,.2);border-radius:10px"><span style="font-size:9px;font-weight:950;letter-spacing:.1em;color:var(--gold);flex:none">GF</span><div style="flex:1;min-width:0"><b style="font-size:10.5px">${esc(g.action)}</b><div style="font-size:9.5px;color:var(--tx2);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(g.why)}${g.sub_hint?` · ${esc(g.sub_hint.why)}`:""}</div></div>${g.order?`<button class="btn sm primary" style="flex:none" onclick="sendOrder('${g.order}')">DO IT ▶</button>`:""}</div>`;
}
...
/* HISTORY — permanent record of finished seasons */
let HIST_SEASON=0, HIST_ALL=false;
async function renderHistory(){
  const j=await api.get("/api/screen/history?season="+HIST_SEASON);
  await refreshState();
  const ch=j.champions||[], prm=j.promoted||[], rel=j.relegated||[], aw=j.awards||[], tbl=j.my_league;
  const empty=!ch.length&&!prm.length&&!rel.length&&!aw.length&&!tbl;
  const chips=j.seasons.map(s=>{
    const on=s===j.season;
    return `<button onclick="HIST_SEASON=${s};HIST_ALL=false;renderHistory()" style="flex:none;min-width:54px;padding:6px 8px;border-radius:10px;font-weight:900;font-size:11px;border:1px solid ${on?"rgba(44,255,138,.35)":"var(--line)"};background:${on?"var(--acc-dim)":"var(--panel2)"};color:${on?"var(--acc)":"var(--tx2)"}">${s}/${String(s+1).slice(2)}</button>`;
  }).join("");
  const champRows=ch.map(c=>{
    const tier=c.tier?` · tier ${c.tier}`:"";
    return `<div style="display:flex;gap:10px;align-items:center;padding:8px;border-radius:10px;background:linear-gradient(135deg,rgba(255,204,51,.07),rgba(255,204,51,.01));border:1px solid rgba(255,204,51,.16)"><div style="width:30px;height:30px;border-radius:9px;background:rgba(255,204,51,.14);border:1px solid rgba(255,204,51,.25);display:grid;place-items:center;font-weight:950;font-size:12px;color:var(--gold);flex:none">T</div><div style="flex:1;min-width:0"><b style="font-size:12px">${esc(c.comp)}</b><div class="small muted" style="font-size:10px">${esc(c.club)}${tier}</div></div><span class="tag" style="font-size:8.5px;background:rgba(255,204,51,.14);color:var(--gold)">WINNER</span></div>`;
  }).join("");
  const move=[...prm.map(x=>({x,t:"up"})),...rel.map(x=>({x,t:"down"}))];
  const moveChips=move.map(m=>{
    const up=m.t==="up";
    return `<span style="font-size:10px;padding:3px 9px;border-radius:999px;background:${up?"rgba(44,255,138,.12)":"rgba(255,59,74,.12)"};color:${up?"var(--acc)":"#ff8a94"};font-weight:800">${up?"▲":"▼"} ${esc(m.x.club)} <span style="opacity:.6">· ${esc(m.x.comp)}</span></span>`;
  }).join("");
  const trows=(tbl&&tbl.rows)?tbl.rows.map(r=>{
    const gd=r.gf-r.ga;
    return `<div style="display:grid;grid-template-columns:26px 1fr 30px 44px 44px;gap:4px;padding:6px 8px;border-radius:8px;background:var(--panel2);font-size:10px;align-items:center"><span style="text-align:center;font-weight:900">${r.pos}</span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:800">${esc(r.name)}</span><span style="text-align:center;color:var(--tx3)">${r.p}</span><span style="text-align:center">${gd>0?"+":""}${gd}</span><b style="text-align:center;font-family:var(--ff-mono)">${r.pts}</b></div>`;
  }).join(""):"";
  const shown=HIST_ALL?trows:trows.slice(0,8);
  const tblBtn=(tbl&&trows.length>8)?`<button class="btn sm" style="margin-top:8px" onclick="HIST_ALL=!HIST_ALL;renderHistory()">${HIST_ALL?"Show top 8":"Show all "+trows.length}</button>`:"";
  const awRows=aw.map(a=>{
    let who="";
    if(a.player){ who=` · <b style="color:var(--tx2)">${esc(a.player)}</b>`; if(a.club){ who+=` (${esc(a.club)})`; } }
    return `<div style="display:flex;gap:8px;align-items:center;padding:8px;border-radius:10px;background:var(--panel2);border:1px solid var(--line)"><div style="flex:1;min-width:0"><b style="font-size:11.5px">${esc(a.name)}</b><div class="small muted" style="font-size:10px;margin-top:1px">${esc(a.detail)}${who}</div></div></div>`;
  }).join("");
  let body;
  if(empty){
    body=`<div class="card" style="text-align:center;padding:26px"><h3>No seasons in the record yet</h3><p class="small muted" style="margin-top:6px">Finished seasons land here with final tables, champions, promotion and player awards — and stay browsable forever.</p></div>`;
  } else {
    const trophy=`<div class="card tight"><div class="sec-h"><h3>Trophy cabinet</h3><span class="small muted">${ch.length} title${ch.length===1?"":"s"}</span></div><div style="display:grid;gap:6px;margin-top:8px">${champRows||'<p class="small muted" style="padding:6px">No trophies recorded for this season.</p>'}</div></div>`;
    const movement=moveChips?`<div class="card tight" style="margin-top:8px"><div class="sec-h"><h3>League movement</h3></div><div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:8px">${moveChips}</div></div>`:"";
    const tableCard=tbl?`<div class="card tight" style="margin-top:8px"><div class="sec-h"><h3>${esc(tbl.comp.name||"League")} — final table</h3><span class="small muted">you: ${tbl.my_pos} · ${tbl.my_pts} pts</span></div><div style="display:grid;gap:3px;margin-top:8px">${shown}</div>${tblBtn}</div>`:"";
    const awards=aw.length?`<div class="card tight" style="margin-top:8px"><div class="sec-h"><h3>Player awards</h3><span class="small muted">${aw.length}</span></div><div style="display:grid;gap:6px;margin-top:8px">${awRows}</div></div>`:"";
    body=trophy+movement+tableCard+awards;
  }
  $("#content").innerHTML=`
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px"><b style="font-size:13px"> HISTORY · ${j.label}</b><span class="spacer"></span><span class="small muted" style="font-size:10px">permanent record</span></div>
    <div style="display:flex;gap:5px;overflow-x:auto;padding-bottom:8px;scrollbar-width:none;margin-bottom:10px">${chips}</div>
    ${body}
...
async function rejectJob(clubId){ const r=await api.post("/api/career/reject",{club_id:clubId}); toast(esc(r.msg||(r.ok?"Offer declined":"No offer from that club")),4000); try{ await refreshState(); if(screen==="career") await renderCareer(); else if(screen==="jobs") await renderJobs(); }catch(e){} }
